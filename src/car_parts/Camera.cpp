...
istream &operator>>(istream &in, Camera &Camera) {
    in >> Camera.model >> Camera.camera >> Camera.RGBFrameResolution
       >> Camera.RGBFrameRate >> Camera.FOV >> Camera.depthFrameRate;
    return in;
}

void Camera::save() {
    ofstream out("Camera" + to_string(time(nullptr)) + ".txt");
    out << "型号：" << this->model << endl;
    out << "摄像头：" << this->camera << endl;
    out << "RGB帧分辨率：" << this->RGBFrameResolution << endl;
    out << "RGB帧率：" << this->RGBFrameRate << endl;
    out << "FOV：" << this->FOV << endl;
    out << "深度帧率：" << this->depthFrameRate << endl;
    out.close();
}
