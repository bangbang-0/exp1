...
void BasePlate::save() {
    ofstream out("BasePlate" + to_string(time(nullptr)) + ".txt");
    out << "编号：" << this->id << endl;
    out << "型号：" << this->model << endl;
    out << "轴距：" << this->wheelBase << "mm" << endl;
    out << "轮距：" << this->wheelTrack << "mm" << endl;
    out << "最小离地间隙：" << this->minGroundClearance << "mm" << endl;
    out << "最小转弯半径：" << this->minTurningRadius << "m" << endl;
    out << "驱动形式：" << this->driveMode << endl;
    out << "最大行程：" << this->maxTravel << "KM" << endl;
    out << "轮胎 (" << tire.size() << "个)" << endl;
    if (!tire.empty()) {
        tire[0].save(out);
    }
    out.close();
