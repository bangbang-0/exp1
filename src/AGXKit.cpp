...
void AGXKit::save() {
    ofstream out("AGXKit" + to_string(time(nullptr)) + ".txt");
    out << "型号：" << this->model << endl;
    out << "AI：" << this->AI << " TOPS" << endl;
    out << "CUDA核心：" << this->CUDA << endl;
    out << "Tensor CORE：" << this->tensorCore << endl;
    out << "显存：" << this->memory << "G" << endl;
    out << "存储：" << this->storage << "G" << endl;
    out.close();
}
