...
void SongLingCar::save() {
    ofstream out("SongLingCar" + to_string(time(nullptr)) + ".txt");
    out << "编号：" << this->id << endl;
    out << "底盘：" << endl;
    this->basePlate.save();
    ...
    out << "双目摄像头(" << this->binocularCamera.size() << "个)：\n";
    for (BinocularCamera x : this->binocularCamera) {
        x.save();
    }
    out << "多线激光雷达(" << this->multiLineLidar.size() << "个)：\n";
    for (MultiLineLidar x : this->multiLineLidar) {
        x.save();
    }
    out << "9轴陀螺仪(" << this->nineAxisGyroscope.size() << "个)：\n";
    ...
    out << "电池模块(" << this->batteryModule.size() << "个)：\n";
